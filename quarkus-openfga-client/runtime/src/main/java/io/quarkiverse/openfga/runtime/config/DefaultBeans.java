...
public class DefaultBeans {
    ...
    public static StoreClient defaultStoreBean(API api,
            @ConfigProperty(name = "quarkus.open-fga.store-id") Optional<String> storeId) {
        return new StoreClient(api, storeId.orElseThrow(
                () -> new ConfigurationException("StoreClient requires a store-id", Set.of("quarkus.open-fga.store-id"))));
    }
    ...
    public static AuthorizationModelClient defaultAutModelBean(API api,
            @ConfigProperty(name = "quarkus.open-fga.store-id") Optional<String> storeId,
            @ConfigProperty(name = "quarkus.open-fga.authorization-model-id") Optional<String> authorizationModelId) {
